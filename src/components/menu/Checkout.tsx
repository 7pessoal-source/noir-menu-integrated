...
import { CartItem, CheckoutData, Neighborhood, DeliveryConfig } from "@/types/menu";
import {
  RESTAURANT_CONFIG as FALLBACK_RESTAURANT_CONFIG,
  DELIVERY_CONFIG as FALLBACK_DELIVERY_CONFIG,
  NEIGHBORHOODS as FALLBACK_NEIGHBORHOODS,
  PAYMENT_METHODS as FALLBACK_PAYMENT_METHODS,
...
export function Checkout({ items, subtotal, onClose, onComplete, neighborhoods, deliveryConfig }: CheckoutProps) {
  const neighborhoodsToUse = neighborhoods || FALLBACK_NEIGHBORHOODS;
  const deliveryConfigToUse = deliveryConfig || FALLBACK_DELIVERY_CONFIG;
  const restaurantConfigToUse = FALLBACK_RESTAURANT_CONFIG; // Podemos expandir isso depois se necessário
  const paymentMethodsToUse = FALLBACK_PAYMENT_METHODS;
  ...
  const validate = (): boolean => {
    ...
    if (!restaurantConfigToUse.schedule.isOpen) {
    ...
    if (deliveryConfigToUse.minimumOrderEnabled && subtotal < deliveryConfigToUse.minimumOrder) {
      newErrors.push(`Pedido mínimo não atingido: ${formatPrice(deliveryConfigToUse.minimumOrder)}`);
    }
  ...
  const generateWhatsAppMessage = (): string => {
    const lines: string[] = [
      "🍽️ *NOVO PEDIDO - " + restaurantConfigToUse.name.toUpperCase() + "*",
      "",
      "📋 *ITENS DO PEDIDO:*",
    ];
    ...
    const paymentName = paymentMethodsToUse.find((p) => p.id === data.paymentMethod)?.name || data.paymentMethod;
  ...
  const handleSubmit = () => {
    ...
    const whatsappUrl = `https://wa.me/${restaurantConfigToUse.whatsappNumber}?text=${message}`;
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-background rounded-2xl border border-border scrollbar-gold">
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {paymentMethodsToUse.map((method) => (
                <button
                  key={method.id}
        ...
        <div className="sticky bottom-0 bg-background border-t border-border p-4 space-y-4">
          ...
          <button
            ...
            className="w-full py-4 rounded-xl bg-primary text-primary-foreground font-bold text-lg gold-glow hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2"
          >
