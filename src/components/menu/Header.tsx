...
import { RESTAURANT_CONFIG as FALLBACK_RESTAURANT_CONFIG, DELIVERY_CONFIG as FALLBACK_DELIVERY_CONFIG } from "@/config/menuConfig";
import { useSupabaseMenu } from "@/hooks/useSupabaseMenu";

export function Header() {
  const { restaurantConfig: supabaseConfig, deliveryConfig: supabaseDelivery } = useSupabaseMenu();
  
  const config = supabaseConfig || FALLBACK_RESTAURANT_CONFIG;
  const delivery = supabaseDelivery || FALLBACK_DELIVERY_CONFIG;
  
  const { name, tagline, schedule } = config;
